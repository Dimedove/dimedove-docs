...
- At least one agent configured with tools you want to expose
...
1. In the **Agent** dropdown, select the agent whose tools you want to expose
...
### Step 4: Select Tools to Expose

After assigning an agent, you can choose which tools to expose:

1. In the **Exposed tools** section, you will see all tools configured on the selected agent
2. Check the boxes next to the tools you want to make available through this MCP Server
3. Only active tools can be selected

External tools will only be able to run the tools you explicitly select here.
...
Leave this disabled if you only want to expose tools without knowledge base access.
...
2. The status indicator shows the result:
   ...
   - **Connected, no tools**: Server works but no tools are exposed
   - **Error**: Configuration issue (check that an agent is assigned and tools are selected)
...
The agent whose tools and knowledge base will be exposed. You can only assign one agent per MCP Server. If you need to expose multiple agents, create separate MCP Servers for each.

### Exposed Tools

Select which of the agent's tools are available through this MCP Server. This gives you fine-grained control over what external tools can do. You might expose read-only API tools while keeping write operations private.
...
- **Not configured**: No agent assigned or no tools selected
- **Connected**: Server is active and tools are available
- **Connected, no tools**: Server works but nothing is exposed (expose tools or enable knowledge base)
...
- At least one tool is selected for exposure

### Connection Test Fails

Check that:
- The agent is live and active
- Selected tools are active (not disabled)
