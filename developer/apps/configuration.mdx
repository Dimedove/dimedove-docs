...
Configure custom tool names for your agent's existing tools, or define client tools that signal your frontend to trigger UI actions during a conversation. See [Custom Tools](/developer/apps/custom-tools) for full setup instructions.
...
- **Custom Tool Mappings**: assign custom names to your agent's existing tools so your frontend can identify them in `function_call` response items.
