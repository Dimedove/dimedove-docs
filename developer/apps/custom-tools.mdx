...
- **Custom Tool Mappings**: assign custom names to your agent's existing tools (API calls, calendar bookings, email sends, etc.) so your frontend can identify them.
...
Your agent can have tools configured in the Dimedove dashboard (HTTP API calls, MCP tool calls, calendar bookings, etc.). By default, these tools surface in the API response under internal names. Custom tool mappings let you assign your own names to them, so your frontend can reliably identify which tool ran.

### Configuring Tool Mappings

1. In the **Configuration** tab of your App, scroll to the **Custom Tools** section.
2. You will see a list of all active tools assigned to the app's agent.
3. For each tool you want to intercept, enter a custom name in the input field next to it.
4. Save the App.
...
When the agent runs a mapped tool, the `function_call` item in the response uses your custom name:
...
The `arguments` field contains the JSON-encoded inputs the agent passed to the tool.
