...
- Written notification to affected customers within seventy-two (72) hours of confirming a security breach affecting customer data
...
- Voice audio for phone-based AI agents is processed in real-time by Twilio (telephony) and ElevenLabs (voice synthesis). Dimedove does not store voice recordings.
- All telephony connections are encrypted in transit using industry-standard protocols
- Only conversation transcripts are retained by Dimedove, encrypted at rest using AES-256
- ElevenLabs receives text content for speech synthesis only, not raw audio
