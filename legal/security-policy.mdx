...
- **Compliance Focus:** Adherence to Canadian privacy laws (including Quebec’s Law 25), PIPEDA, and industry standards
...
- Written notification to affected customers of a confirmed security breach affecting customer data, provided with diligence and within the time required by applicable law. Any shorter notification period agreed in writing with a customer continues to apply
...
- In accordance with Quebec’s Law 25, all confidentiality incidents (breaches or security events involving personal information) are logged and maintained in an internal register
...
- Dimedove services and data are hosted in the AWS Canada Central region (ca-central-1)
...
- Data in transit is encrypted using commercially reasonable, industry-standard transport encryption
- API and application endpoints are TLS/SSL only, with HTTP requests redirected to HTTPS
- HTTP Strict Transport Security (HSTS) is enforced, including on subdomains
...
- Customer data is encrypted at rest using commercially reasonable, industry-standard controls, where appropriate
- Encryption keys are managed through our cloud provider's managed key services
...
- Groq, Cerebras, and Baseten: High-performance AI inference
- Deepgram and ElevenLabs: Speech recognition for voice agents
- Cartesia: Speech synthesis for voice agents
...
- **List:** [Dimedove Subprocessors](https://docs.dimedove.com/legal/subprocessors-list)
...
- Compliance with Canadian privacy laws including Law 25 and PIPEDA
...
If you discover a potential security vulnerability in Dimedove's platform, we encourage responsible disclosure:

- Report vulnerabilities to **security@dimedove.com** with a detailed description of the issue, steps to reproduce, and any supporting evidence
- We will acknowledge receipt of your report within two (2) business days
- Our security team will investigate and work to address confirmed vulnerabilities promptly
- We ask that you allow reasonable time for remediation before disclosing the vulnerability publicly
...
- Dimedove does not train AI models on identifiable Customer Data, and identifiable customer conversation content is not used to train third-party or general-purpose AI models. We select AI providers whose terms, to our knowledge, do not permit them to use the data we send them for model training. Any model development, tuning, or evaluation carried out by Dimedove uses aggregated, anonymized, or properly de-identified data, as described in Section 5.6 of the [Terms of Service](https://docs.dimedove.com/legal/terms-of-service)
...
- All telephony connections are encrypted in transit using industry-standard protocols
- Conversation transcripts are retained by Dimedove, encrypted at rest using industry-standard controls
- Call recording is off by default; when a Business Customer turns it on for a phone line, recordings are stored in Canada and encrypted at rest
