...
- ElevenLabs: Voice synthesis and text-to-speech
...
- Identifiable customer conversation content is not used to train third-party or general-purpose AI models, and our AI provider agreements prohibit the use of customer conversation data for model training. Any model development, tuning, or evaluation carried out by Dimedove uses aggregated, anonymized, or properly de-identified data, as described in Section 5.6 of the [Terms of Service](https://docs.dimedove.com/legal/terms-of-service)
- Safeguards in place against adversarial prompts and injection attacks

**Voice and Telephony Security:**

- Voice audio for phone-based AI agents is carried by Twilio (telephony) and processed in real time by ElevenLabs, which provides speech recognition, speech synthesis, and call handling for the voice agent. Dimedove does not store voice recordings.
- All telephony connections are encrypted in transit using industry-standard protocols
- Only conversation transcripts are retained by Dimedove, encrypted at rest using industry-standard controls
- Voice recording is disabled on Dimedove's voice agent configuration
