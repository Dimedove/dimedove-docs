...
- **Routing condition**: an optional rule that tells the agent when to route to this host (shown when you configure more than one host).
- **Meeting length and buffers**: per-host duration, buffer time before and after, and minimum advance notice.
- **Event title**: the title applied to the event in Outlook.
- **Use as default**: the host used when no routing condition matches (shown when you configure more than one host).
...
- **Scoped access**: your agent checks availability, books, and reschedules only on the hosts you configure.
...
1. Navigate to **Settings > Integrations** in your Dimedove dashboard, or open the connection card from within a **Calendar** tool.
2. Click **Connect**.
3. Sign in as a Microsoft 365 administrator and grant Dimedove consent for your organization. Dimedove requests permission to read and write calendars, read mailbox settings (working hours), and read your organization's user directory (to pick mailboxes).
4. You are returned to the dashboard, where the integration shows as connected.

To set up the calendar tool:

1. Navigate to your agent's **Tools** and add a **Calendar** tool.
2. Select **Outlook** as the provider.
3. Add the host(s) the agent can book. For each host, choose a mailbox, give it a label, and set the meeting length, buffers, minimum notice, and event title. Add a routing condition when you use more than one host.
4. Save and test.

<Info>
  Outlook Calendar connects in live calendar mode. Calendar tools are part of Dimedove's agent tool capabilities. See the [Calendar tool documentation](/platform/agents/tools/calendar) for full setup details.
</Info>
