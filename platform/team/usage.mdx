...
2. **Tool Completion**  
   The conversation counts when the agent successfully runs a specific tool (e.g., _book a demo_, _request a quote_, _handoff to broker_). The tool is selected from the list of tools defined in the agent configuration.
...
- **Every Qualified Opportunity includes an explanation** (e.g., _"Captured Budget + Timeline + Needs"_ or _"Completed tool: Book demo"_) for transparency.
- **Performance metrics must accurately reflect business objectives.** The chosen performance metric directly impacts your agent's conversational direction, focus areas, and operational efficiency. Selecting an inappropriate metric may lead to suboptimal agent behavior. Refer to our [Terms of Service](/legal/terms-of-service) for details on performance metric requirements.
